...
const Teste: React.FC<IRouteParams> = ({ match }) => {
  ...
  async function Submit() {
    ...
    const response = await api.get(
      "/report?date=" + format(startDate, "ddMMyyyy")
    );
  ...
  return (
    <Container>
      ...
      <Content>
        {isLoading ? (
          <ClipLoader color={"#ffffff"} size={50} loading={isLoading} />
        ) : (
              ...
              {produtos.map((produto) => (
                <div key={produto.id}>
                  <div>
                    {" "}
                    {(produto.color === "red" && (
                      <p className="redaa">{produto.number}</p>
                    )) ||
                      (produto.color === "black" && (
                        <p className="reda11">{produto.number}</p>
                      )) || (
                        <>
                          <p className="mostrar">Qtd até cair</p>
                          <p className="reda22">{produto.rounds_to_white}</p>
                        </>
                      )}
                  </div>
